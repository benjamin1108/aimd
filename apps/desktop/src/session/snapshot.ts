...
export function loadSessionSnapshot(): SessionSnapshot | null {
  try {
    ...
    return {
      ...
      assets,
      docuTour: parsed.docuTour && typeof parsed.docuTour === "object" ? parsed.docuTour : null,
...
export function persistSessionSnapshot() {
  ...
  const snapshot: SessionSnapshot = {
    ...
    docuTour: state.doc.docuTour || null,
...
export async function restoreSnapshot(snapshot: SessionSnapshot): Promise<{ doc: AimdDocument; mode: Mode; message: string } | null> {
    ...
    try {
      ...
      return {
        doc: {
          ...
          html,
          assets: diskDoc.assets,
          docuTour: diskDoc.docuTour || snapshot.docuTour || null,
          dirty: true,
          isDraft: false,
          format: "aimd",
        },
  ...
  return {
    doc: {
      ...
      docuTour: snapshot.docuTour || null,
